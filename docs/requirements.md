...
- 必須項目 `filePath`, `format`, `width`, `height`, `deliveryPreset` の不足を検出する。
...
参照元 ZIP: `D:\AI\WindowsApp\created_idea_004_media-delivery-preflight-suite\idea_004_media-delivery-preflight-suite.zip`。一部に文字化けがあったため、正式docsは `PICKUP_100_IDEAS.md` とドメインガイドから再構成した。
